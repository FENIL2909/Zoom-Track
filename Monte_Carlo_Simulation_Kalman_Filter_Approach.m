    ...
    for time = 1:(Timer_Step_Change + T_operation)
            ...
            if ((detectFlag_A(Target_no) == 1) && (detectFlag_AA(Target_no) ==1) &&(detectFlag_AAA(Target_no)==0)&&(Target(Target_no)<=1500))
                ...
                if Authenticated_Target < K
                    Authenticated_Target = Authenticated_Target + Add_Target;
                end
...
xlabel('Number of simulations'),ylabel('Percentage of Success'),title('Kalman Filter Success Graph - V=25 units')
grid on

figure(2)
plot(Step_no,Failed_TARGET,'o')
xlabel('Number of simulations'),ylabel('Percentage of Failure'),title('Kalman Filter Failure Graph - V=25 units')
grid on
